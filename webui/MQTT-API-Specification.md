...
**Purpose:** Real-time voltage/current monitoring and general-purpose I/O using DA/DB pins. DA/DB pins are settable per pin as either digital (0V/3.3V) or analog (0–3.3V). The data stream reports voltages only (no per-pin mode differentiation).
...
```json
{
  ...
  "payload": {
    ...
    "settings": {
      ...
      "signal": { "ch0": 1.000, "ch1": 2.500 },
      "da": [
        { "mode": "digital", "value": 0 },
        { "mode": "analog",  "value": 1.200 },
        { "mode": "digital", "value": 3.300 },
        { "mode": "analog",  "value": 0.000 }
      ],
      "db": [
        { "mode": "digital", "value": 3.300 },
        { "mode": "digital", "value": 0 },
        { "mode": "analog",  "value": 2.500 },
        { "mode": "analog",  "value": 0.100 }
      ]
    }
  }
}
...
```json
{
  ...
  "payload": {
    ...
    "readings": {
      ...
      "input_ch0": 2.456,
      "input_ch1": 1.789,
      "da": [3.300, 0.025, 1.650, 0.000],
      "db": [0.000, 3.300, 2.500, 0.100]
    },
...
- Update interval: 50ms to 1000ms
 - Signal outputs (ch0/ch1): 0.000V to [Signal Output Range], typically up to ~13.7V (DAC 2.048V × gain 6.7)
 - DA/DB config array lengths: up to 4 entries each
 - Per-pin modes: "digital" | "analog"
 - For "digital", `value` is treated as 0V or 3.3V (any non-zero maps to 3.3V)
 - For "analog", `value` is a float in 0.000–3.300 volts
 - Legacy: `{ "level": 0|1 }` accepted instead of `value`

**Pin Naming Notes:**
- DA0–DA3: MCU-direct lines, settable per-pin as digital (0/3.3V) or analog via PWM (0–3.3V). Mapped to GPIO6,7,8,9.
- DB0–DB3: General-purpose lines, settable per-pin as digital (0/3.3V) or analog via PWM (0–3.3V). Mapped to GPIO33,34,35,36.
